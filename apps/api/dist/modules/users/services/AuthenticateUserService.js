...
let AuthenticateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const notAuthMessage = 'Incorrect email or password.';

    if (!user) {
      throw new _AppError.default(notAuthMessage, 401);
    }

    const passwordMatched = await this.hashProvider.comapreHash(password, user.password);
